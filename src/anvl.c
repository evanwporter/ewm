...
void tag_next_mon(Seat *seat, Arg *arg) {
  if (selmon != NULL && seat->focused != NULL) {
    ...
    if (next != NULL && &next->link != &anvl.outputs) {
      ...
      insert_node(seat->focused, next->tags[next->seltag]->root,
                  next->tags[next->seltag]->focused);
    }
  }
}
...
void tag_prev_mon(Seat *seat, Arg *arg) {
  if (selmon != NULL && seat->focused != NULL) {
    ...
    if (prev != NULL && &prev->link != &anvl.outputs) {
      remove_node(seat->focused->node);
      insert_node(seat->focused, prev->tags[prev->seltag]->root,
                  prev->tags[prev->seltag]->focused);
    }
  }
}
...
void focus_next(Seat *seat, Arg *arg) {
  if (seat->focused != NULL) {
    Window *next =
        wl_container_of(seat->focused->link.next, seat->focused, link);
...
void focus_prev(Seat *seat, Arg *arg) {
  if (seat->focused != NULL) {
    Window *prev =
        wl_container_of(seat->focused->link.prev, seat->focused, link);
...
void tag(Seat *seat, Arg *arg) {
  if (seat->focused != NULL) {
    remove_node(seat->focused->node);
    insert_node(seat->focused, selmon->tags[arg->u]->root,
                selmon->tags[arg->u]->focused);
  }
}
...
void propogate_layout(Node *root) {
  ...
  while (front != back) {
    ...
    if (n->first != NULL && n->second != NULL) {
      ...
      n->second->x =
          n->split_type == VERTICAL ? n->x + n->first->width + gappx : n->x;
      n->second->y =
          n->split_type == HORIZONTAL ? n->y + n->first->height + gappx : n->y;
...
void river_output_v1_wl_output(void *data, struct river_output_v1 *obj,
                               uint32_t name) {
  Output *output = data;
...
void river_output_v1_position(void *data, struct river_output_v1 *obj,
                              int32_t x, int32_t y) {
  Output *output = data;
...
void river_output_v1_dimensions(void *data, struct river_output_v1 *obj,
...
void river_window_v1_dimensions_hint(void *data, struct river_window_v1 *obj,
                                     int32_t min_width, int32_t min_height,
                                     int32_t max_width, int32_t max_height) {}

void river_window_v1_dimensions(void *data, struct river_window_v1 *obj,
                                int32_t width, int32_t height) {
...
void river_window_v1_decoration_hint(void *data, struct river_window_v1 *obj,
                                     uint32_t hint) {}
void river_window_v1_pointer_move_requested(void *data,
                                            struct river_window_v1 *obj,
                                            struct river_seat_v1 *river_seat) {}
void river_window_v1_pointer_resize_requested(void *data,
                                              struct river_window_v1 *obj,
                                              struct river_seat_v1 *river_seat,
                                              uint32_t edges) {}
void river_window_v1_show_window_menu_requested(void *data,
                                                struct river_window_v1 *obj,
                                                int32_t x, int32_t y) {}
void river_window_v1_maximize_requested(void *data,
                                        struct river_window_v1 *obj) {}
void river_window_v1_unmaximize_requested(void *data,
                                          struct river_window_v1 *obj) {}
void river_window_v1_fullscreen_requested(
    void *data, struct river_window_v1 *obj,
    struct river_output_v1 *river_output) {}
void river_window_v1_exit_fullscreen_requested(void *data,
                                               struct river_window_v1 *obj) {}
void river_window_v1_minimize_requested(void *data,
                                        struct river_window_v1 *obj) {}
void river_window_v1_unreliable_pid(void *data, struct river_window_v1 *obj,
                                    int32_t unreliable_pid) {}
void river_window_v1_presentation_hint(void *data, struct river_window_v1 *obj,
                                       uint32_t hint) {}
void river_window_v1_identifier(void *data, struct river_window_v1 *obj,
...
void window_set_dimensions(Window *window, int width, int height) {
  window->width = width;
  window->height = height;
  river_window_v1_propose_dimensions(window->river_window, window->width,
                                     window->height);
}

void river_xkb_binding_v1_pressed(void *data,
                                  struct river_xkb_binding_v1 *obj) {
  Key *key = data;
  key->func(key->seat, key->arg);
}

void river_xkb_binding_v1_released(void *data,
                                   struct river_xkb_binding_v1 *obj) {}
...
void xkb_binding_create(Seat *seat, uint32_t modifiers, xkb_keysym_t keysym,
                        void (*func)(Seat *seat, Arg *arg), Arg *arg) {
  ...
  river_xkb_binding_v1_add_listener(key->river_xkb_binding,
                                    &xkb_binding_listener, key);
...
void river_pointer_binding_v1_pressed(void *data,
...
void river_pointer_binding_v1_released(void *data,
                                       struct river_pointer_binding_v1 *obj) {
  Button *button = data;
...
void pointer_binding_create(Seat *seat, uint32_t modifiers, uint32_t ibutton,
                            void (*func)(Seat *seat, Arg *arg), Arg *arg) {
  Button *button = calloc(1, sizeof(Button));
  button->river_pointer_binding =
      river_seat_v1_get_pointer_binding(seat->river_seat, ibutton, modifiers);
  button->seat = seat;
  button->func = func;
  button->arg = arg;

  river_pointer_binding_v1_add_listener(button->river_pointer_binding,
                                        &pointer_binding_listener, button);
...
void river_seat_v1_pointer_enter(void *data, struct river_seat_v1 *obj,
...
void river_seat_v1_window_interaction(void *data, struct river_seat_v1 *obj,
                                      struct river_window_v1 *river_window) {
  Seat *seat = data;
...
void river_seat_v1_op_delta(void *data, struct river_seat_v1 *obj, int32_t dx,
                            int32_t dy) {} // Used for dragging
void river_seat_v1_op_release(void *data, struct river_seat_v1 *obj) {
} // Used for dragging

void river_seat_v1_pointer_position(void *data, struct river_seat_v1 *obj,
                                    int32_t x, int32_t y) {
  Output *output;
  wl_list_for_each(output, &anvl.outputs, link) {
    if (x >= output->x && x < output->x + output->width && y >= output->y &&
...
void river_layer_shell_output_v1_non_exclusive_area(
    void *data, struct river_layer_shell_output_v1 *handle, int32_t x,
...
void river_window_manager_v1_unavailable(void *data,
                                         struct river_window_manager_v1 *obj) {
  fprintf(stderr, "error: Unavailable.\n");
  exit(1);
}

void river_window_manager_v1_finished(void *data,
                                      struct river_window_manager_v1 *obj) {
  exit(0);
}
...
void tile(Output *output) {
  ...
  while (front != back) {
    ...
    if (n->first != NULL && n->second != NULL) {
      ...
      window_set_dimensions(n->window, n->window->node->width,
                            n->window->node->height);
...
void river_window_manager_v1_manage_start(void *data,
                                          struct river_window_manager_v1 *obj) {
  Seat *seat;
...
void render_chars(const char *chars, size_t len, int x, int y, int width,
                  int (*fx)(int, int, int), pixman_image_t *pix,
  ...
  for (size_t i = 0; i < len; i++) {
    ...
    pixman_image_composite32(PIXMAN_OP_OVER, color, g->pix, pix, 0, 0, 0, 0,
                             cx + g->x, y + fcft_font->ascent - g->y, g->width,
                             g->height);
...
void render_bar(WlOutput *output) {
  ...
  pix = pixman_image_create_bits_no_clear(PIXMAN_a8r8g8b8, w, h,
                                          (void *)pool_data, stride);
  if (pix == NULL) {
...
void river_window_manager_v1_render_start(void *data,
                                          struct river_window_manager_v1 *obj) {
  WlOutput *output;
...
void river_window_manager_v1_window(void *data,
                                    struct river_window_manager_v1 *obj,
                                    struct river_window_v1 *river_window) {
...
void river_window_manager_v1_output(void *data,
                                    struct river_window_manager_v1 *obj,
                                    struct river_output_v1 *river_output) {
  Output *output = calloc(1, sizeof(Output));
  output->river_output = river_output;
  output->river_layer_shell =
...
void river_window_manager_v1_seat(void *data,
                                  struct river_window_manager_v1 *obj,
                                  struct river_seat_v1 *river_seat) {
  ...
  for (int i = 0; i < LENGTH(keybinds); i++) {
    xkb_binding_create(seat, keybinds[i].mods, keybinds[i].key,
...
struct river_xkb_keymap_v1 *create_keymap() {
  ...
  return river_xkb_config_v1_create_keymap(xkb_config, keymap_fd,
...
void wl_registry_global(void *data, struct wl_registry *registry, uint32_t name,
  ...
  if (strcmp(interface, river_input_manager_v1_interface.name) == 0) {
    ...
    if (input_manager != NULL) {
      river_input_manager_v1_add_listener(input_manager,
                                          &input_manager_listener, NULL);
    }
  }
  ...
  if (strcmp(interface, river_xkb_config_v1_interface.name) == 0) {
    ...
    if (xkb_config) {
      ...
      if (xkb_keymap) {
        river_xkb_keymap_v1_add_listener(xkb_keymap, &xkb_keymap_listener,
                                         NULL);
      }
    }
  }
